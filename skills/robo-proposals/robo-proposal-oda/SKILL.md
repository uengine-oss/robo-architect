...
2. **전략 분해** — 표준에 정렬된 Strategic Diff(BoundedContext(=Epic)/Feature/UserStory)를
   표준 Proposal Diff 계약(tempId+참조)대로
...
```json
{
  ...
  "strategicDiff": { "...": "표준 Proposal Diff 형식(version, boundedContexts/features/userStories, tempId+refs)" },
  "journeys": []
}
```
...
```json
{
  ...
  "tacticalDiff": [ {"...": "표준 tacticalDiff 형식(tempId+refs)"} ],
  "artifacts": {
