...
- **빈 배열에는 입증 책임이 있다.** `[]` 는 "안 찾아봤다"가 아니라 "찾아봤는데 없다"를 뜻한다.
  비우기 전에 `legacy-reference.md` **S3 배분**(후보 목록을 후보→요소 방향으로 재훑기)과
  **S4 갭마감**(빈 요소만 1회 재검색)을 거친다. 특히 Command/Event/ReadModel 처럼 상위 축
  검색에 이미 나왔을 수 있는 세부 요소는 **이름이 아니라 요약으로** 대응을 판단하라.
...
```
응답에 있던 문장: "포인트 사용액이 보유 잔액을 초과하면 사용을 거절한다."
  ✅ "rule": "포인트 사용액이 보유 잔액을 초과하면 사용을 거절한다."
  ❌ "rule": "use_point > member.point_balance -> RET_DENIED(잔액부족)"   ← 코드로 변환
  ❌ "rule": "잔액 부족 시 거절"                                          ← 요약
```

문장을 그대로 옮길 자신이 없으면 `rule` 을 생략하고 함수 `nodeId` 만 근거로 남겨라 —
기각되는 인용보다 정확한 함수 근거가 낫다.
