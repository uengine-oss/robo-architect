...
7. **Constitution/Plan 검토(041)** — 입력에 프로젝트 Constitution 과 Implementation Plan 이 주어지면, 작업 분해는 그 아키텍처(아키텍처 스타일·레포 매핑·연동 방식/메시징 채널·서비스별 개발환경)와 **일관**되어야 한다. 예: 마이크로서비스 + repo-per-service 이면 서비스별 작업으로 묶고, 각 서비스의 Docker 개발환경/의존(예: Kafka) 셋업 작업을 포함한다. Constitution 결정과 **충돌하는 작업**은 조용히 만들지 말고 작업 설명에 `⚠️ Constitution 충돌:` 로 명시하라.
