...
2. **Event Discovery (10 minutes)**: Brain dump all domain events
3. **Event Refinement (15 minutes)**: Validate and improve events
4. **Commands & Policies (15 minutes)**: Add commands and policies
5. **Timeline Ordering (10 minutes)**: Arrange chronologically
6. **Summary (5 minutes)**: Review and document
...
async def get_session_instructions(session_id: Optional[str] = None) -> str:
    ...
    phase_instruction = PHASE_INSTRUCTIONS.get(
        session.phase, PHASE_INSTRUCTIONS[SessionPhase.ORIENTATION]
    )
...
def get_phase_transition_message(from_phase: SessionPhase, to_phase: SessionPhase) -> str:
    ...
    messages = {
        (SessionPhase.ORIENTATION, SessionPhase.EVENT_ELICITATION): "Great! Now let's discover domain events. Remember: past tense only! What important things HAPPEN in your domain?",
        (SessionPhase.EVENT_ELICITATION, SessionPhase.EVENT_REFINEMENT): "Wonderful brainstorming! Now let's refine these events. I'll help check if they follow the rules.",
        (SessionPhase.EVENT_REFINEMENT, SessionPhase.COMMAND_POLICY): "Events look good! Now let's add Commands (what triggers events) and Policies (what reacts to events).",
        (SessionPhase.COMMAND_POLICY, SessionPhase.TIMELINE_ORDERING): "Excellent! Now let's organize everything on a timeline. Drag events from left (first) to right (last).",
        (SessionPhase.TIMELINE_ORDERING, SessionPhase.SUMMARY): "Great work organizing! Let me summarize what we've discovered today.",
    }
