...
<rule id="5">**Never name a context after an activity.** A bare gerund or verb-noun — "Closing", "Granting", "Approval", "Settlement", "Aggregation", "Reconciliation" — names something the business *does*, not an area it *owns*. Such a name has no subject, so the aggregates extracted under it have nothing to be about, and they come back as the activity restated ("Closing" gives "MonthlyClosing", "AnnualLeaveGranting" gives "AnnualLeaveGrant"). Name the subject the activity acts on: closing and aggregating monthly attendance is the **Attendance** context; granting and expiring annual leave is the **LeaveEntitlement** context.</rule>
<rule id="6">**Do not name a context after a system boundary.** "...Integration", "...Interface", "...Gateway", "...Adapter", "...Sync" describe how two systems are wired, not a domain the business reasons about. Payroll hand-off belongs to the context that owns the data being handed over.</rule>
<rule id="7">**One context per subject, not per process step.** A single business process usually crosses one or two contexts, not one per phase. If your contexts line up one-to-one with the steps of a single document, you have split a workflow rather than found boundaries — merge the ones that change the same data. Requesting leave and approving leave are the same subject moving through its lifecycle; daily attendance and monthly closing are the same subject at two granularities.</rule>
</section>
...
4. **Merge contexts that are phases of one subject's lifecycle, or two granularities of the same data.** A context named after an activity — a bare gerund or verb-noun like "Closing", "Granting", "Approval", "Settlement", "Aggregation" — is almost always a phase of something else's life, not a domain of its own. It loses to the context that owns the subject it acts on: "Closing" absorbs into "Attendance", "Approval" into the context owning the thing being approved, "AnnualLeaveGranting" into the context owning the entitlement. Likewise merge a daily-grain context with the monthly-grain context over the same data. When you keep the subject's name over the activity's, say so in the rationale.
5. **A context named after a system boundary is not a domain.** "...Integration", "...Interface", "...Gateway", "...Adapter", "...Sync" describe wiring between systems. Absorb them into the context that owns the data being exchanged.
6. Only merge BCs that are genuinely semantically similar — do NOT merge unrelated BCs just to hit the target count
...
<rule id="7">**Do not restate the Bounded Context.** The context names the area; the Aggregate names the thing inside it whose state changes. If your Aggregate name is the context name with a word bolted on — "Closing" giving "MonthlyClosing", "AnnualLeaveGranting" giving "AnnualLeaveGrant", "PayrollIntegration" giving "PayrollInterface" — you have not found an entity, you have echoed the heading. **This happens most when the context itself is named after an activity, and it is the single most common way this step goes wrong.** Ask instead: when this context does its work, *what* changes? Closing changes an employee's attendance for a month — the Aggregate is the attendance, and "closed" is one of its states. Granting changes how much leave a person is entitled to — the Aggregate is the entitlement, and the grant is an event on it.</rule>
<rule id="8">**A context should usually hold more than one Aggregate.** One Aggregate per context, across every context, means the contexts were drawn at the size of a single entity — or that each Aggregate is just the context restated. Look again for the other things in this area that have their own identity and lifecycle: a leave context holds both the request and the balance it draws down; an attendance context holds both the daily record and the monthly roll-up. One Aggregate is legitimate for a genuinely small context, but not as the pattern.</rule>
</section>
...
<rule id="6">**Never the activity itself.** "Closing", "Granting", "Approval", "Settlement", "Integration", "Interface" name work or wiring, not a subject. If the surrounding Bounded Context carries such a name, do not inherit it — the context was named badly and rule 7 of "What Is Not an Aggregate" applies.</rule>
