...
def _compact_inspection(node_id: str, data: dict, raw_head: str) -> dict:
    ...
    # Architect's normal detail path persists Analyzer's validated source-free frame packet
    # verbatim.  No complete source, source excerpt or hash crosses this boundary.
    if (data.get("schema_version") == "semantic-frame-packet/v1"
            and isinstance(node.get("semantic_frame"), dict)):
        labels = list(node.get("labels") or [])
        return {
            "nodeId": node_id,
            "ok": True,
            "view": "frame",
            "schemaVersion": str(data.get("schema_version") or ""),
            "name": node.get("name", ""),
            "label": next((label for label in labels if label != "EMBEDDED"), ""),
            "semanticFrame": dict(node.get("semantic_frame") or {}),
            "ruleExamples": list(node.get("rule_examples") or []),
            "linkedContext": dict(node.get("linked_context") or {}),
        }
