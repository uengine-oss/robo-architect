...
def _build_prompt(state: dict) -> str:
    ...
    return (
        ...
        "[DECISION RULES]\n"
        "- 원문을 입력→검증→분기→CALL/RW→반환/상태변화→transaction 순서로 이해한다.\n"
        "- Then은 분기 중간 대입값이 아니라 공통 후처리·clamp·rollback/commit까지 적용한 최종 결과다. "
        "범위 조건만으로 최종 상수 반환을 일반화하지 않는다.\n"
        "- GWT fieldValues는 RULE/원문/실제 sample에 있는 필드와 값만 사용한다. 근거가 없으면 빈 맵이다.\n"
        ...
        "- 각 Command.legacyRefs에 근거 함수 nodeId, 그 함수의 실제 RULE text 1개 이상, packet.tables의 "
        ...
        '"properties":[],"userStoryRefs":["us:<allowed-id>"],"gwt":[{"scenario":"...","given":{"name":"Aggregate: ...",'
