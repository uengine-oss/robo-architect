...
def _build_prompt(state: dict) -> str:
    ...
    has_evidence = has_grounded_legacy_evidence(packet)
    packet_use_rule = (
        "packet의 ordered_flow와 RULE condition/effects를 입력→검증→분기→CALL/RW→최종 결과 순서로 "
        "연결하고, 사용한 exact evidence ID만 기록한다."
        if has_evidence else
        "packet이 없으므로 Original Requirement와 Define UserStory만으로 설계한다. 레거시 의미나 "
    ...
    prompt = (
        ...
        "조건은 name에 쓰고 fieldValues는 비운다. Command당 시나리오는 1~4개이며, 경계/실패는 "
        "요구사항 또는 packet에 직접 근거가 있을 때만 추가한다. 개수를 맞추려고 만들거나 복제하지 않는다.\n"
...
def _has_complete_commands(
    artifact: dict, allowed_story_ids: set[str], *, require_evidence_refs: bool = False,
) -> bool:
    commands = [
        command
...
async def stream(proposal_id: str, feedback: str = None) -> AsyncGenerator[tuple[str, object], None]:
    ...
    async for ev in execute_stage(
        ...
        blocking_validators=[(
            ...
            "모든 Command에 허용된 userStoryRefs와 1~4개의 근거 있는 구조화 GWT가 필요합니다",
        ), (
