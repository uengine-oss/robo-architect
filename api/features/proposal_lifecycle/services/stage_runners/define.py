...
def _build_prompt(state: dict) -> str:
    ...
    return (
        ...
        "packet은 semantic slots와 profile RULE condition·effects를 우선 읽고, source를 재조회하지 않은 채 "
        "줄 좌표와 missing_context로 모호함을 드러낸다. slot meaning은 profile 구조 사실에 결속된다. "
        "근거가 부족하면 UserStory 인수조건에 값이나 상태를 만들지 않고 openQuestions에 남긴다.\n\n"
