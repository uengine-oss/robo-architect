...
async def confirm_wizard_step(
    ...
    for change_id, change in accepted.items():
        try:
            if change.targetType == "BoundedContext" and change.action == "create":
            ...
            elif change.targetType == "BoundedContext" and change.action == "update" and change.targetId:
                _apply_bc_update(change.targetId, change.after)
                applied.append(change_id)
                ...
                bc_id = change.after.get("boundedContextId")
                if bc_id:
    ...
    return WizardConfirmResponse(appliedChanges=applied, errors=errors)
