...
User Story (source_screen_name = "주문 목록")
    ...
    ↑ fuzzy matching으로 검증/보정 (bigram Jaccard similarity ≥ 0.5)

Neo4j UserStory node (sourceScreenName = "주문 목록")
    ↑ create_user_story()에서 영속화 → sync_from_neo4j()에서 복원

Command (user_story_ids = ["US-003"])
    ↑ identify_bounded_contexts → extract_commands에서 연결

UI Wireframe 생성 프롬프트
    ↑ _resolve_figma_screen_ref()에서 source_screen_name 조회
      → 정확 매치 시도 → 실패 시 fuzzy fallback → 로깅
      → ctx.figma_screens[resolved_name] 을 프롬프트에 주입
```

### 5.3 source_screen_name 검증 (Fuzzy Matching)

LLM이 추론한 `source_screen_name`은 다음 2단계에서 검증된다:

**단계 1: User Story 추출 직후 (`figma_to_user_stories._validate_source_screen_names`)**

```python
# 매칭 우선순위:
1. 정확 일치 (exact match)
2. 대소문자 무시 일치 (case-insensitive)
3. 부분 문자열 포함 (substring match)
4. Bigram Jaccard 유사도 ≥ 0.5
```

- 보정 성공 시 → `source_screen_name` 자동 수정 + INFO 로그
- 보정 실패 시 → WARN 로그 (원본 값 유지, UI 생성 시 무시됨)

**단계 2: UI Wireframe 생성 시 (`ui_wireframes._resolve_figma_screen_ref`)**

동일한 fuzzy matching을 2차로 적용하여, US 추출 이후 화면 이름이 변경되었거나
워크플로우 재개 시에도 매핑이 동작하도록 보장한다.

- 매칭 성공 → 프롬프트에 Figma 화면 구조 주입
- 매칭 실패 → WARN 로그 + Figma 참조 없이 UI 생성 (silent fail 방지)

### 5.4 source_screen_name Neo4j 영속화

`source_screen_name`은 Neo4j `UserStory` 노드의 `sourceScreenName` 속성으로 저장된다:
...
`sync_from_neo4j()` 호출 시 자동 복원되며, `figma_screens` 맵도 `ctx.content`(Figma JSON)에서 재구성된다.
이를 통해 워크플로우 일시정지/재개 후에도 Figma 화면 참조가 유지된다.
...
├── figma_to_user_stories.py            ← Figma 전용 US 추출 (청킹, 프롬프트, fuzzy matching)
...
│   └── user_stories.py                 ← create_user_story (sourceScreenName 영속화)
└── workflow/
    ├── ingestion_workflow_context.py    ← figma_screens 맵 + sync_from_neo4j 복원
    └── phases/
        ├── parsing.py                  ← Figma 메시지 분기
        ├── user_stories.py             ← Figma 청킹 처리 + figma_screens 저장 + 검증
        └── ui_wireframes.py            ← _resolve_figma_screen_ref (fuzzy fallback + 로깅)
```
...
- `source_screen_name` 매핑은 LLM 추론 + fuzzy matching 보정이지만, 극단적으로 모호한 이름은 여전히 오매핑 가능

### 구현 완료

- ~~화면 이름 유사도 기반 fuzzy matching으로 매핑 보정~~ → `_fuzzy_match_screen_name()` (bigram Jaccard)
- ~~source_screen_name Neo4j 미저장~~ → `UserStory.sourceScreenName` 속성으로 영속화
- ~~매칭 실패 시 silent fail~~ → `_resolve_figma_screen_ref()`에서 WARN 로그 + fuzzy fallback
- ~~워크플로우 재개 시 figma_screens 소실~~ → `sync_from_neo4j()`에서 `ctx.content`로부터 재구성
...
- 스타일 정보(색상, 폰트, 간격) 추출하여 UI wireframe 품질 향상
