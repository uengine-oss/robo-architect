...
> **알아 둘 결합 하나.** 매니페스트의 `credentialNames` 는 *"이 릴리스가 값을
> 실었을 뻔한 이름"* 에서 나온다 — 즉 **릴리스를 굽는 기계의 `.env` 에 있던
> 것**만 목록에 오른다. 빌더가 `AUTH_JWT_SECRET` 을 안 채우고 구우면 그 이름이
> 목록에서 빠지고, 설치본은 그것을 요구하지 않는다(= 재기동마다 세션이 끊기는
> 옛 동작으로 조용히 돌아간다). 릴리스 관문이 `AUTH_ENFORCE` 가 켜졌을 때
> `AUTH_JWT_SECRET` 을 요구하는 것이 이 구멍을 막는 유일한 자리다.
>
> 계약(`credentialNames`) 전체를 무조건 요구하게 만들면 `AUTH_DEV_LOGIN_PASSWORD`
> 처럼 진짜로 선택인 것까지 강제된다. 그래서 "실었을 뻔한 것만" 으로 뒀다.
