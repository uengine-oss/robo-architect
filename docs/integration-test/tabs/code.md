...
### S5. 외부 핸드오프 — ✅ (C6/핸드오프-cwd 수정)
- Changes/Proposals "구현 시작"·PRD 열기 → `openClaudeCode` → Code 탭. 메커니즘 동작 확인(앞서 Changes 검증). **버그 2건 수정**(C6/S5): 콜드스타트 명령 누락 + 다른 프로젝트 핸드오프 시 cwd 미추종(C7 핸드오프 짝). 새/다른-workdir 세션은 respawn+initialCommand 로 새 cwd에서 ~6s 후 실행.

### S6. 세션 수명주기 — 🟡 코드확인 (풀 재현=Proposals 탭)
...
| C6/S5 | 🟢 **(수정)** | **외부 핸드오프(openClaudeCode) 2버그**: ① 명령 자동실행 안 됨(콜드스타트 레이스) ② **다른 프로젝트로 핸드오프 시 터미널 cwd 가 안 따라감**(C7와 동일 클래스, 핸드오프 경로 — 내 C7 폴더피커 픽스가 미커버). PRD 열기·프로젝트 전환 시 PTY 가 옛 cwd 유지. | [_onTerminalOpen main](../../../frontend/src/features/claudeCode/ui/ClaudeCodeWorkspace.vue#L347)가 (a) `main.workdir` 만 repoint, PTY respawn 안 함 (b) `nextTick sendInput(cmd+\n)` 이 claude 부팅 전 실행돼 누락. | **수정함**: 새 메인 세션·다른-workdir 핸드오프는 **respawn(epoch++)+`initialCommand`** 사용 → 터미널이 claude 부팅 후(~6s) 새 cwd 에서 명령 실행. 같은 workdir 기존 세션은 sendInput 유지. (C7 폴더피커 픽스의 핸드오프 짝.) |
...
**검증 범위**: S0~S6 라이브(사용자 UI 조작 + 그래프/MCP/FS 교차검증). robo-spec 프로젝트(`/Users/seongwon/projects/my-project`)를 모델에서 생성해 end-to-end.

**통과**: S0(진입·워크디렉터리)·S1(파일 트리/에디터/저장/삭제/fs-events)·S2(터미널 PTY·C11 정정)·S3(재어태치)·**S4(MCP/robo-spec, end-to-end)** ·S5(외부 핸드오프). S6=코드확인(풀 재현은 Proposals).

**S4 하이라이트**: 모델 → `/robo-plan`(설계 fetch+분류+plan.md) → `/robo-tasks`(22/22) → `/robo-implement`(Clean Arch 코드, 1agg·3cmd·7evt·3rm 정확 반영) → `register_implementation_files`(그래프 역등록, cmd 2파일·evt 7/7) → Design 탭 진척동기화. MCP 읽기+쓰기 도구 전부 동작, 그래프=단일진실.

**수정한 버그**:
- **D7 🔴 P0(수정)** — get_bc_design 이 APOC 미설치로 빈 설계 반환 → robo-spec 무력화. APOC 의존 제거. **이 픽스가 S4 전체를 가능케 함.**
...
**교차**: 외부 핸드오프=Changes(038)·Proposals(039)(C6/S5 픽스로 cwd 정상), MCP=robo_spec(D7), 진척동기화=Stories/Design 탭(소스 점프), 세션 수명주기 I14=Proposals 탭에서 마저.
