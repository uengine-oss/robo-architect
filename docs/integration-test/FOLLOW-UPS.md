...
| **하이브리드 BPM·EM 게이트웨이 누락** | 🟠 **판단 확정(2026-06-19 검증)** | **하이브리드 BPM 뷰가 일직선만 그림** — 분기(게이트웨이)가 안 보임. 근데 데이터엔 분기가 풍부함: **Command가 Event 2개+ EMITS = 분기 27개**(예: 카드 발급사 확인→4 events). **3개의 다른 게이트웨이 면**이 얽혀 있음: ① 하이브리드 BPM 뷰는 `BpmProcess.bpmn_xml`(pdf2bpmn 산출, gateway 0개) 렌더 ② ES process-flow(`_generate_bpmn_xml`, `len(evt_ids)>1`→게이트웨이)는 27개 분기 있으나 **하이브리드 뷰가 안 씀** ③ EM NEXT_UI 게이트웨이(025)는 하이브리드 미생성(`ctx.content=""`라 화면흐름 추출 입력 없음). **핵심 코드증거**: 하베스트 파서 [`parse_bpmn_xml_per_process`](../../api/features/ingestion/hybrid/document_to_bpm/a2a_adapter.py#L277)가 **Task+sequenceFlow만 추출, 게이트웨이 파싱 안 함**(L308-322) → BpmSequence가 선형 task 리스트로 평탄화. 즉 extractor가 게이트웨이를 만들어도 **받아오는 단계에서 버림**. | **✅검증완료(2026-06-19, session 05ac3bb9 실데이터 + PDF 원문 + 코드)**: "a2a가 게이트웨이를 만들어줌에도 받아오질 못한다"는 **사실로 확정**. ① **PDF 소스는 명백히 분기 프로세스**(이전 메모의 "5장 선형" 추정은 **오류**) — `요청처리.pdf` 5장 To-Be Flow가 **2단계 "결제수단별 처리 경로 결정"(은행/카드/간편결제 3-way)·7단계 "본인확인 방식 결정"(필요/면제/자체 3-way)·3·4·5·6단계 거부/생략 분기**를 본문에 직접 기술. ② **추출기(pdf2bpmn)는 게이트웨이 생성 능력 보유** — `neo4j_client.create_gateway`/`HAS_GATEWAY`, `entity_extractor`("Gateways: Decision/branching POINTS 분기점"), [`pdf2bpmn_agent_executor.py:5483`]가 **분기점마다 ExclusiveGateway 강제 삽입**. ③ **robo가 버리는 지점 = 4겹(코드 확정)**: (a) harvest [`_harvest_bundle_from_pdf2bpmn_neo4j`](../../api/features/ingestion/hybrid/document_to_bpm/a2a_adapter.py#L417)가 `(:Process)-[:HAS_TASK]->(:Task)-[:PERFORMED_BY]->(:Role)`만 읽고 **`:Gateway`/`HAS_GATEWAY` 미조회** — 작성자 주석(L131-132)도 "plus **gateways**/events"가 쓰여있음을 인정하면서 안 읽음 (b) XML분기 `parse_bpmn_xml_per_process`도 Task+sequenceFlow만 (c) [`bpmn_builder.py:4`](../../api/features/ingestion/hybrid/document_to_bpm/bpmn_builder.py#L4) **"Intentionally simple — gateways/branches are out of scope for v0.1"**(선형 StartEvent→task→EndEvent 생성) (d) relabel은 `:Gateway`→`:BpmnGateway` 보존하나 **소비자 코드 0건**(grep: schema 정의+relabel 쓰기뿐) → orphan. ④ **실데이터 실측**: 저장 bpmn_xml이 robo 재생성본(`id=Definitions_hybrid`, task 12 선형 chain, gateway 0), `BpmnGateway`=0·`gateway_type` 속성 노드 0·`:Event` 41건 전부 ES(event_type 없음). **미확정 1점(snapshot 불가)**: 이번 실행에서 추출기가 bolt Neo4j에 `:Gateway`를 **실제로 썼는지**(robo harvest가 버림=B1) vs AGE에만 쓰고 bolt엔 Process/Task만 도달(B2) — 단 (c) build_bpmn_xml이 의도적 선형이라 **어느 쪽이든 결과 동일**. 정확한 손실지점 확정 = **라이브 재인제스천 + raw 로깅**(추출기의 bolt `:Gateway` write 카운트 + A2A 응답 payload) 필요. **수정 경로**: (A) 하베스트+빌더에 게이트웨이 파싱/생성 추가 — 가장 깔끔 (B) 이미 있는 ES 분기 27개를 다운스트림 주입(`bpmn_xml`+EM NEXT_UI) — 042 "단일그래프 두 투영"과 합치, PDF/추출기 불변 (C) pdf2bpmn 추출 개선(외부 a2a 영역). EM 게이트웨이 파생도 같은 분기 소스 → A/B로 BPM·EM 양쪽 동시 해결. **★라이브 재인제스천(2026-06-19, session 2af336bd)로 B1/B2 결판**: A2A 추출기가 **이 환경에선 기여 0** — ① 추출기 백엔드 AGE/Postgres(`:5432` `password authentication failed for user "postgres"`)·입력 메멘토(`:8005` 연결실패) **둘 다 다운**이라 추출기 즉시 예외 ② **A2A 서버가 실패를 은폐** — `{status:"completed", message:"BPMN extraction completed successfully", artifacts:[]}` 반환(실제론 실패) ③ robo가 빈 번들 감지 → [`__init__.py:130-135`](../../api/features/ingestion/hybrid/document_to_bpm/__init__.py#L130) **"falling back to native"** → [`extract_bpm_from_document`](../../api/features/ingestion/hybrid/document_to_bpm/entity_extractor.py#L114)(네이티브 LLM 추출기)가 생성. **네이티브 추출기는 게이트웨이 개념 자체가 없음**(entity_extractor.py:49·92 "tasks: …linear order"/"Ordered linearly") → `build_bpmn_xml` 선형 렌더. **결론**: 이 환경의 BPM(선형 12+7, gw 0)은 **A2A가 아니라 robo 네이티브 폴백 산출**이고, 원래 05ac3bb9도 동일 구조(9050≈9051B)라 네이티브 폴백 추정. 게이트웨이를 실제로 얻으려면 **(1) 게이트웨이 소스 확보**(A2A 인프라 메멘토:8005+AGE:5432 복구 → 실 추출기 가동, 또는 네이티브 추출기에 분기 모델링 추가) **+ (2) robo 수신측 수정**(harvest가 `:Gateway`/`HAS_GATEWAY` 읽기 + build_bpmn_xml 게이트웨이 출력, 또는 경로 B). **별도 버그**: A2A 서버가 추출 예외인데 "completed/성공" 응답 → 실패가 조용히 네이티브 폴백으로 마스킹됨(진단 어려움). |
...
| **EM 추가 노드가 Design에서 겹침** | 🟠 | EM(Event Modeling)에서 추가한 노드(Command/Event/ReadModel/UI)가 **Design 탭에도 반영되어 어느 정도 자기 위치는 잡히나, 기존 노드들과 위치가 너무 겹침**. | **원인**: Design 노드 좌표는 **그래프에 저장 안 됨**(Command 등에 posX/x/position 속성 0; keys=name/desc/actor/id/displayName/category/inputSchema 등). Design 캔버스([CanvasWorkspace.vue](../../frontend/src/features/canvas/ui/CanvasWorkspace.vue), Vue Flow)가 **`addNodesWithLayout` auto-layout**으로 배치하고 **BC(boundedcontext) 위치만 sync/보존**(L609-647). 신규 EM-추가 노드는 auto-layout이 **기존 배치와 충돌 회피를 못 해** 같은 자리에 겹쳐 렌더. **수정 방향**: (a) Design add-node 시 **빈 슬롯/충돌회피 배치**(기존 노드 bbox 회피) (b) 신규 노드만 부분 재레이아웃 (c) EM-add가 Design 좌표 힌트 제공(단 좌표 미저장 모델이라 a/b가 자연스러움). Design 탭 정식 검증 시 함께. |
