...
/**
 * Feature edit dialog (034 — US3).
 * Renames / re-describes a Feature via PATCH /feature. Child User Stories
 * stay attached (relationships preserved server-side).
 */
const props = defineProps({
  feature: { type: Object, required: true }, // { id, name, description }
})
...
async function save() {
  if (!name.value.trim()) {
    errorMsg.value = 'Feature 이름은 비울 수 없습니다.'
    return
  }
  busy.value = true
  errorMsg.value = ''
