<template>
  <div class="tviz">
    ...
    <section v-for="group in groupedAggregates" :key="group.name" class="tviz__group">
      <h4 class="tviz__group-title">{{ group.name }}</h4>
      <div v-for="(a, i) in group.items" :key="a.name || i" class="adc">
        <section class="adc__cell adc__name">
          <h5>Name</h5>
          <input v-model="a.name" placeholder="Aggregate" />
          <span class="adc__bc-badge">{{ bcName(a) }}</span>
        </section>
...
const groupedAggregates = computed(() => {
  const byName = new Map()
  for (const aggregate of aggregates.value) {
    const name = bcName(aggregate)
    if (!byName.has(name)) byName.set(name, { name, items: [] })
    byName.get(name).items.push(aggregate)
  }
...
function bcName(a) {
  return a.boundedContextName || a.bcName || a.boundedContext || a.contextName || a.context || a.bcId || '미지정 BC'
}
...
.tviz__group { margin-bottom: 18px; }
.tviz__group-title { margin: 0 0 8px; font-size: 13px; color: var(--color-text-bright); border-bottom: 1px solid var(--color-border); padding-bottom: 4px; }
...
.adc__bc-badge { display: inline-flex; margin-top: 5px; font-size: 10px; border-radius: 999px; padding: 2px 7px; background: var(--status-blue-bg); color: var(--status-blue-fg); }
