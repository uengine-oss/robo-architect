...
const SHOTS = resolve(__dirname2, '../../specs/038-change-management/manual/screenshots')
...
test('02 — Change 상세 화면 (CHG-012)', async ({ page }) => {
  ...
  // CHG-012 span을 직접 클릭
  const firstChange = page.locator('span.cp-item__id').filter({ hasText: 'CHG-012' }).first()
