...
- [ ] No [NEEDS CLARIFICATION] markers remain  *(FR-010 carries one marker: server-side fallback language when no Accept-Language is provided. Resolve via `/speckit-clarify`)*
...
- One outstanding [NEEDS CLARIFICATION] in FR-010 (server-side default language fallback). All other items pass. Address via `/speckit-clarify` before `/speckit-plan`.
- The "language-policy as cross-cutting concern" framing (US3, FR-008, FR-015, SC-005) is the spec's load-bearing design choice — flag it explicitly during clarification if reviewers think the single-chokepoint requirement is too strong.
