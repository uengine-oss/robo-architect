...
- **FR-010**: When an inbound request carries no Language indicator (no `Accept-Language` header or equivalent), the backend MUST apply a server-side default Language. The chosen default is **`en-US`** (broadest reader audience for technical/developer-facing callers), overridable per-deployment via the `GENERATION_LANGUAGE_DEFAULT` environment variable. (Resolved in [research.md §D2](./research.md); a single-language team can flip the default without code changes.)
