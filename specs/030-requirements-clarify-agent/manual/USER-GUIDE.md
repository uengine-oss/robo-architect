...
![명확화 레이더 차트 — 비기능 축이 0%로 함몰 (Outstanding · 적색 점), 엣지 케이스는 50% Deferred (황색)](images/00-clarity-radar.png)

### 3.1.1 SKILL.md 기반 평가 모델 — 4-상태 weighted

레이더의 카테고리별 점수는 SpecKit `/speckit-clarify` 스킬의 **step-8
coverage 모델**과 동일한 4-상태 체계를 가중치로 사용합니다:

| 상태 (skill 정의) | 점의 색 | 점수 | 의미 |
|---|---|---|---|
| **Clear** ("already sufficient") | 🟢 초록 | **1.0** | 스캔 결과 충분히 명세됨 — 갭 없음 |
| **Resolved** ("was Partial/Missing and addressed") | 🔵 파랑 | **1.0** | 이번 세션에서 갭이 발견되었지만 적용으로 해결 (sticky — 다음 스캔에서 다시 Outstanding이 돼도 유지) |
| **Deferred** ("exceeds quota OR better suited for planning") | 🟡 황색 | **0.5** | 갭이 발견됐지만 질문 5개 cap을 초과해 보류 — *알고 있지만 후순위* |
| **Outstanding** ("still Partial/Missing but low impact") | 🔴 적색 | **0.0** | 갭이 발견됐지만 해결되지 않은 상태 — *가장 우선 검토 대상* |

스킬 인용:

> Coverage summary table listing each taxonomy category with Status:
> Resolved (was Partial/Missing and addressed), Deferred (exceeds
> question quota or better suited for planning), Clear (already
> sufficient), Outstanding (still Partial/Missing but low impact).

이 상태는 **딥 에이전트가 매 스캔 종료 시 직접 결정**합니다 — 스캔 단계
(step 2)의 `Clear / Partial / Missing` 라벨링과 **Impact × Uncertainty**
우선순위 결정(step 3)을 거친 결과입니다. 백엔드는 그 결과를 in-memory
`clarification_coverage` 캐시에 보관해 다음 레이더 갱신 때 사용합니다.

### 3.1.2 점수 산출 우선순위

1. **에이전트 coverage 캐시가 있으면** — 4-상태 가중치 그대로 사용.
   (Single source of truth: 에이전트가 결정한 상태)
2. **캐시가 없으면 (cold-start)** — in-memory 플래그 트래커로 fallback.
   플래그 있으면 0.0 (Outstanding 취급), 없으면 1.0 (status=None — 회색).

### 3.1.3 Resolved의 sticky 보장

스킬의 "Resolved = was Partial/Missing and addressed" 정의를 충실히 따르기
위해, 한 번 Resolved가 된 카테고리는 후속 스캔에서 같은 카테고리가 다시
Outstanding이나 Deferred로 보고돼도 *다운그레이드하지 않습니다*. 새 갭이
같은 카테고리에서 발견됐다면 그건 *새* 작업이지, 과거 작업을 무효화한
것은 아닙니다.
...
- **헤더의 카운터**: "1건 모호 / 2건 전체" 처럼 범위 내 전체 UserStory 수
  대비 현재 모호로 판정된 수.
- **축 라벨/점 hover**: 카테고리 enum 이름, 정확 점수(%), **상태 라벨**
  (Clear/Resolved/Deferred/Outstanding), flagged·resolved 카운트.
- **범례 (하단)**: 4가지 상태 색의 의미.

### 3.1.5 자동 갱신 시점

- Requirements 탭 마운트 시
- 명확화 세션의 스캔이 끝났을 때 (coverage 캐시 갱신)
- `/apply` (해당 카테고리를 Resolved로 sticky 업그레이드) 또는 `/revert` 직후
- 인제스트가 완료되거나 데이터가 초기화될 때

> **활용 팁**: 가장 안쪽으로 들어간 축(0.0 적색 = Outstanding)을 우선
> 검토하세요. 황색 Deferred은 인지된 갭이지만 cap에 밀려 보류된 것이므로
> 두 번째 우선순위입니다. 같은 범위에서 한 번 더 스캔을 돌리면 cap에 밀린
> Deferred 카테고리들이 차례로 올라옵니다.
