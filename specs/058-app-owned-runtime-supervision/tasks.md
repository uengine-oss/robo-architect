...
- [X] T048 [US6] 권한 격리를 데이터로 판정한다 — 좁혀진 role 로 접속해 **심어 둔 라벨을 세어** 0건인지 본다. `MATCH (n)` 은 권한이 없어도 0건이라 증거가 못 된다 (맥)
- [X] T049 [US6] 기존 구성에 남은 사용자 데이터의 이관 가능 여부를 기동 시 알린다 — 조용히 빈 화면이 되면 안 된다(FR-030) (공통)

> **T048·T049 실측 (2026-09-18, 맥)**
>
> ### T048 — "0 건"이 언제 나오는지 정확히 쟀다
>
> 이 태스크의 전제는 "`MATCH (n)` 은 권한이 없어도 0건이라 증거가 못 된다" 였다.
> 재어 보니 **이 빌드에서는 그렇지 않다.**
>
> ```
> 권한이 아예 없는 role   데이터 있는 graph   → **거부** (0 건이 아니다)
> 권한이 아예 없는 role   빈 graph            → **거부**
> 다른 graph 에 권한이 있는 role → 남의 graph 의 **있는 라벨**  → 거부
>                                 남의 graph 의 **없는 라벨**  → 0 (정상 부재)
> ```
>
> 즉 0 이 나오는 자리는 **"그 graph 에 그 타입의 테이블이 없다"** 하나뿐이고, 그건
> 누수가 아니다. 그래도 그 갈래를 통과로 치면 **빈 graph 에 권한이 새는 것**을 못
> 잡는다 — 그것을 짝으로 확인했다.
>
> ```
> 권한 없는 role 에게 **빈 graph** read 를 준다 (실재하는 누수)
>   0 건도 실패로 보는 판정   → exit 1, 그 graph 를 이름으로 지목
>   0 건을 통과로 보는 판정   → exit 0 **통과**  ← 거짓 통과
>   검사를 건너뛴 판정        → exit 0 **통과**  ← 거짓 통과
> ```
>
> `scripts/verify_role_isolation.py` 에 그 두 갈래를 더했다(권한 없는 role ×
> 데이터 있는 graph · 빈 graph). 기존 세 갈래(정상 부재 / 거부 / 샘)는 그대로다.
>
> ### T049 — 업그레이드한 사용자가 "지워진 줄" 알게 된다
>
> 두 구성이 **볼륨이 다르다**(`neo4j_data` ↔ `graph_data`). 옛 데이터는 지워지지
> 않지만 **새 앱이 읽지 않는다.** 그러면 프로젝트 목록이 그냥 비어 있고 오류도 없다.
>
> `desktop/src/main/legacy-data.ts` 가 기동 시점에 가른다.
>
> ```
> 옛 볼륨 있음 + 새 저장소 비었음   needsAttention. "지워진 것이 아닙니다" 를 먼저 말하고
>                                   되돌리는 문서를 가리킨다
> 옛 볼륨 있음 + 새 저장소에 데이터  **경고가 아니다.** 정상 상태에 빨간불을 켜 두면
>                                   사용자가 빨간불을 무시하는 법을 배운다
> 옛 볼륨 없음                      할 말이 없다
> 못 물어봤다(`null`)               경고하지 않고 **못 쟀다는 사실만** 남긴다
> ```
>
> `RuntimeState.legacyData` 로 렌더러까지 올라간다.
>
> ### 검사
>
> `legacy-data.spec.ts`(9) — 데스크톱 단위 **84건** 통과. 결함 다섯을 심어 각각
> 해당 검사만 무는 것을 확인했다(못 쟀다를 없다로 읽기 · 옮긴 뒤에도 경고 · 모를 때
> 경고 · 할 일을 안 주기 · 정렬 안 하기).
>
> **안 쟀다**: T050(화면에서 로그인·프로젝트 선택)은 (윈). 실제 업그레이드 경로 —
> Neo4j 구성으로 데이터를 만든 뒤 새 릴리스를 덮는 것 — 도 Windows 몫이다.
