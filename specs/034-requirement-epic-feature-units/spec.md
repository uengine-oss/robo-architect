...
- **FR-032**: 사용자가 동의하면 시스템은 **기존 인제스천의 설계 단계를 그대로** 해당 US에 실행해야 한다 — 즉 **이벤트 도출 → Aggregate → Command → ReadModel** 워크플로 정순으로, **기존 인제스천 진행 다이얼로그·SSE 루프·UI를 동일하게** 사용한다. (요구사항↔설계 갭 메우기는 문서 신규 인제스천과 동일한 로직·UI·순서이며, 신규 문서는 갭이 전부라 "처음부터 설계"처럼 보일 뿐이다.)
- **FR-033**: 설계 갭 메우기는 **대상 US가 속한 BC만** 처리해 기존 설계를 보존하고(그래프 clear 금지, MERGE 키로 재사용), 기존 인제스천과 동일하게 진행 상황을 스트리밍해야 한다.
...
**설계 커버리지 검증·복구 (인제스천 사후 — US7 정확도 보강)**

- **FR-036**: "설계 미반영" 판정은 US가 **어떤 behavioral 설계객체(Command/ReadModel/Event/Policy)에도 `IMPLEMENTS`로 연결되지 않은** 경우로 한다. (Command 부재만 보면, 인제스천이 Aggregate엔 배치했으나 Command는 일부만 매핑한 US — 특히 조회성 — 까지 과대 보고되어 잘못된 반영 프롬프트가 뜨고 중복 객체 생성을 유발한다.)
- **FR-037**: 시스템은 **인제스천 사후 검증 체크리스트**(BC별 고아 US 집계)를 제공해야 한다 — 인제스천이 US↔Command/ReadModel `IMPLEMENTS` 링크를 누락(실측: ReadModel에 US 0건 연결)했는지 검증.
- **FR-038**: 시스템은 고아 US를 **기존 Command(액션)/ReadModel(조회)에 매핑해 `IMPLEMENTS` 링크를 복구**해야 하며, **새 설계객체를 만들지 않아** 중복을 피하고, 적합한 기존 객체가 없으면 **unmapped(진짜 공백)로 남겨 리포트**해야 한다.
- **FR-039**: 이 검증·복구는 **인제스천 워크플로 종료 시 자동(best-effort)** 으로 수행되어야 하며(실패해도 인제스천을 중단시키지 않음), 수동 재실행도 가능해야 한다.
