...
**Status**: Draft · **Fix 2026-06-16** (프로젝트 루트 경로 표시 버그)

> 🔧 **2026-06-16 버그수정**: 최근 프로젝트 루트 자동 표시 시 부모 경로가 `/`로 깨지고 basename이 전체경로로 나오던 버그(Windows 역슬래시 경로를 프론트가 `split('/')`로만 분리). **근본 수정**: `projectRoot:validate` IPC가 choose와 동일하게 백엔드 Node `path`로 `basename`/`parent`를 반환(OS 구분자 무관)하고, 프론트(`LauncherView.revalidateRoot`)는 그 값을 사용 — 프론트 문자열 분리 제거(DRY 단일 소스). 계약 변경: `ProjectRootValidateResult`에 `basename?`/`parent?` 추가(valid일 때).
