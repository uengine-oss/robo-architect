...
This is a multi-component project (backend `api/`, frontend `frontend/`, plus the new verbatim-copy distribution root `skills/robo-spec/` at the repo root). Paths below are repo-relative.
...
- [x] T001 Create `skills/robo-spec/` directory at repo root with subtree `skills/robo-spec/{robo-plan,robo-tasks,robo-implement,robo-sync}/` and `skills/robo-spec/robo-sync/extractors/` per [plan.md](plan.md) Project Structure
...
- [x] T003 [P] Create [skills/robo-spec/robo-sync/extractors/package.json](skills/robo-spec/robo-sync/extractors/package.json) declaring `@typescript-eslint/typescript-estree` as the sole runtime dep for the TS AST helper
...
**Independent Test**: Wire any new project to a scratch workspace via `setup-project`; confirm `<workspace>/.claude/skills/` matches `skills/robo-spec/` byte-for-byte (no Jinja markers), and `<workspace>/.claude/{robo-project.json,mcp.json}` exist and point at the running backend. Run quickstart S1 to validate.
...
- [x] T013 [US6] Extend `POST /api/claude-code/setup-project` in [api/features/claude_code/router.py](api/features/claude_code/router.py) to (a) `shutil.copytree(<repo>/skills/robo-spec/, <workspace>/.claude/skills/, dirs_exist_ok=True)`, (b) write `<workspace>/.claude/robo-project.json` per data-model §2.1, (c) write `<workspace>/.claude/mcp.json` with the project-specific `{backendUrl}/mcp` URL, (d) compute and return a sha256 `roboSpecChecksum` of the copied subtree, (e) reject `projectId` mismatch on re-run with HTTP 409 — all per [contracts/http-api.md](contracts/http-api.md) E1
- [x] T014 [P] [US6] Create skeleton [skills/robo-spec/robo-plan/SKILL.md](skills/robo-spec/robo-plan/SKILL.md): frontmatter only (`name: robo-plan`, `description`, `extends: speckit-plan`, `requires-speckit: ">=0.8.13, <0.9.0"`, `user-invocable: true`) + a `## Inheritance` header pointing at `.claude/skills/speckit-plan/SKILL.md` + a `## Overrides` heading with a TODO marker for US1 to flesh out — per research R11
- [x] T015 [P] [US6] Create skeleton [skills/robo-spec/robo-tasks/SKILL.md](skills/robo-spec/robo-tasks/SKILL.md) with the analogous frontmatter (`extends: speckit-tasks`) and inheritance header; `## Overrides` is a TODO for US2
- [x] T016 [P] [US6] Create skeleton [skills/robo-spec/robo-implement/SKILL.md](skills/robo-spec/robo-implement/SKILL.md) with analogous frontmatter (`extends: speckit-implement`); `## Overrides` is a TODO for US4
- [x] T017 [P] [US6] Create skeleton [skills/robo-spec/robo-sync/SKILL.md](skills/robo-spec/robo-sync/SKILL.md) — *no* `extends:` (self-contained per research R11) — with frontmatter (`name: robo-sync`, `requires-speckit: ">=0.8.13, <0.9.0"`, `user-invocable: true`) and a TODO body for US5
- [x] T018 [P] [US6] Create skeleton extractor stubs [skills/robo-spec/robo-sync/extractors/python_extract.py](skills/robo-spec/robo-sync/extractors/python_extract.py) and [skills/robo-spec/robo-sync/extractors/ts_extract.mjs](skills/robo-spec/robo-sync/extractors/ts_extract.mjs) (argument-parsing stubs that exit 1 with "TODO" — full implementation in US5)
- [x] T019 [US6] Add an install-integrity check script at [scripts/check_robo_spec_install.sh](scripts/check_robo_spec_install.sh) that (a) `diff -r skills/robo-spec/ <target>/.claude/skills/` returns no differences, (b) `grep -r -E '\{\{|\{%' skills/robo-spec/` returns nothing (no Jinja markers), and (c) the recorded `roboSpecChecksum` equals the recomputed sha256
...
- [ ] T044 [US5] Implement [skills/robo-spec/robo-sync/extractors/python_extract.py](skills/robo-spec/robo-sync/extractors/python_extract.py) (stdlib `ast`): walk class definitions + dataclass fields + type annotations; emit one JSON document per element `{ kind, name, fields: [{name, type}] }` to stdout; the calling skill is responsible for resolving each name to an `elementId` via the design fetched from T2
- [ ] T045 [US5] Implement [skills/robo-spec/robo-sync/extractors/ts_extract.mjs](skills/robo-spec/robo-sync/extractors/ts_extract.mjs) using `@typescript-eslint/typescript-estree` (per T003): walk interfaces / type aliases / class members; emit the same JSON shape as T044; document the `node extractors/ts_extract.mjs <file.ts>` invocation
...
- [ ] T053 [P] Update the repo [README.md](README.md) architecture section to document the new `skills/robo-spec/` distribution root, the verbatim-copy install behavior, and the existence of the `/robo-*` skill suite
...
Task: "Create skeleton skills/robo-spec/robo-plan/SKILL.md (T014)"
Task: "Create skeleton skills/robo-spec/robo-tasks/SKILL.md (T015)"
Task: "Create skeleton skills/robo-spec/robo-implement/SKILL.md (T016)"
Task: "Create skeleton skills/robo-spec/robo-sync/SKILL.md (T017)"
