...
- `/robo-sync` structural extractors (invoked via Bash and shipped under `skills/robo-spec/robo-sync/extractors/`): TypeScript; Python (currently an explicit skeleton); dependency-free Java source parsing; Avro JSON parsing. Proposal validation supplies the repository extractor directory as `EXTRACTOR_ROOT`, so an already-created sandbox uses the current Java/Avro extractors without being rebuilt.
...
└── robo-spec/
    ...
    └── robo-sync/                          # no speckit counterpart — self-contained
        ...
        └── extractors/                     # client-side AST extractors (run via Bash in the workspace)
            ...
            ├── ts_extract.mjs              # TypeScript class-field extractor
            ├── java_extract.py             # Java fields/methods/events/enums
            └── avro_extract.py             # Avro record fields/nullability/defaults
